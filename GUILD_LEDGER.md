...
**And then a second playtest changed the presentation layer.** The menu-driven interface
is not how this ships: the game opens on the guild hall itself, seen at a high
three-quarter angle, panned with a finger, with townsfolk walking in to take seats under
a 90-second countdown, staff moving between rooms and adventurers leaving on contracts
through the front door. `Docs/World_View_Design.md` is the charter for it and supersedes
the presentation half of the art brief. **The economy, the assemblies, the saves and the
tests are untouched** — the finding that made it affordable is that the tuned economy is
already seat-shaped: `MAX_TURNS_PER_HOUR` is 40, so a seat is occupied for exactly 90
seconds and **never turns over faster at any tier**, and seats are the binding constraint
at every tier, so there is always a queue outside the door. The view depicts the economy
rather than causing it, and the depiction is arithmetically exact. Grey-box first, art
last, exactly as Days 1-13 did.
